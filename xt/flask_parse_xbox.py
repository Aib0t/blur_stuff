...
def string_decryption(encrypted_string:bytes) -> bytes:
    ...
    for byte in encrypted_string:
        ...
        magic_var1 = magic_var1.to_bytes(4,byteorder='big',signed=True)[0]
        i += 1

        temp = temp.to_bytes(4,byteorder='big',signed=True)[0]
        #print(temp)
...
def data_structures_parser(data_structures_data:bytes,data_structures_size:int):
    ...
    for data_structure in data_structures_items:
        ...
        temp_dict["position"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["num_of_items"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["var3"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["var4"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["var5"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["size"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        #print(temp_dict)
...
def data_types_def_parser(data_type_def_data:bytes,data_type_def_size:int):
    ...
    for data_type in data_types_def_parser_items:
        ...
        temp_dict["position"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["var2"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["offset"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["var4"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        #print(temp_dict)
...
def entities_data_parser(entities_data:bytes,entities_data_size:int):
    ...
    for entity in entities_items:
        ...
        # temp_dict["var1"] = int.from_bytes(temp_reader.read(2),byteorder='big')
        # temp_dict["var2"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        ...
        temp_dict["internal_id"] = int.from_bytes(id_bytes, byteorder='big')
        temp_dict["internal_id_hex"] = id_bytes.hex()
        temp_dict["var_type"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["parent"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        #print(temp_dict)
...
def entities_names_data_parser(entities_names_data:bytes,entities_names_data_size:int):
    ...
    for entities_name in entities_names_items:
        ...
        temp_dict["id"] = int.from_bytes(temp_reader.read(4), byteorder='big')
        temp_dict["unknown1"] = int.from_bytes(temp_reader.read(4), byteorder='big')
        temp_dict["offset"] = int.from_bytes(temp_reader.read(4), byteorder='big')
        temp_dict["size"] = int.from_bytes(temp_reader.read(4), byteorder='big')
        #print(temp_dict)
...
def block3_parser(block3_data:bytes,block3_size:int):
    ...
    for value in block3_values:
        ...
        temp_dict["id"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["unknown1"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        temp_dict["unknown2"] = int.from_bytes(temp_reader.read(2), byteorder='big')
        # temp_dict["zeroes"] = int.from_bytes(temp_reader.read(4), byteorder='big')
        # print(temp_dict)
...
class json2_gen:
    ...
    def generate_schema(self):
        ...
        for data_struct in self.block1_parsed:
            ...
            temp_dict['internal_id_hex'] = data_struct['internal_id'].to_bytes(4,'big').hex()
...
def data_cooker(flask_file_reader:io.BytesIO):
    ...
    data_blocks_meta["data_structure_def_pos"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')
    data_blocks_meta["data_structure_def_size"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')

    data_blocks_meta["unknown1_pos"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')
    data_blocks_meta["unknown1_size"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')

    data_blocks_meta["data_types_definitions_pos"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')
    data_blocks_meta["data_types_definitions_size"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')

    data_blocks_meta["encrypted_string_pos"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')
    data_blocks_meta["encrypted_string_size"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')

    data_blocks_meta["entities_data_pos"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')
    data_blocks_meta["entities_data_size"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')

    data_blocks_meta["entities_names_data_pos"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')
    data_blocks_meta["entities_names_data_size"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')

    data_blocks_meta["unknown_block3_position"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')
    data_blocks_meta["unknown_block3_size"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')

    data_blocks_meta["data_block_position"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')
    data_blocks_meta["data_block_size"] = int.from_bytes(flask_file_reader.read(4), byteorder='big')
    ...
    for entity in entity_list:
        ...
        for element in temp_entity_dict['elements']:
            ...
            if read_type == 0:
                ...
                temp_output_entity_dict['data'][element['label_text']]['value'] = int.from_bytes(got_bytes, byteorder='big')
                ...
                temp_output_entity_dict['data'][element['label_text']]['count'] = int.from_bytes(got_bytes,byteorder='big')
        ...
        f = open('online_settings_bad_parse.json', 'w')
...
def main():
    file = open("./blur2/vehicles.bin","rb").read()
